// ** This is the header file for reading and writing the message into/from the image ** //

#pragma pack(2) //pack struct members to 2 bytes alignment
#include <fstream>
#include <iostream>
...
bool ReadBitmapImage(const char* fileName,
...
bool WriteBitmapImage(const char* fileName,
	unsigned char*** imageData,
	int imageWidth,
	int imageHeight) {
...
void ReleaseMemory(unsigned char*** imageData, int imageHeight, int imageWidth) {
	for (int rows = 0; rows < imageHeight; rows++) {
		...
		for (int columns = 0; columns < imageWidth; columns++) {
		
				delete imageData[rows][columns];   // delete the memory allocated by imageData [rows][columns]
		}
		delete imageData[rows];      // delete the memory allocated by imageData[rows]
	}
	delete[] imageData;     // delete the memory allocated by pointer imageData
}
